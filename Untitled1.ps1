...
# NA REBOOT NAKIJKEN VAN ONZE PROGRESS VIA AANGEMAAKTE LOGFILE
$thirdcheck1 = Get-Content $logfile | Where-Object { $_.Contains("AD-DOMAIN SERVICES CONFIGURATION REQUIREMENTS MET, INITIALISING REBOOT, INITIALISING SCRIPT 3 =-") }

#DNS SERVER TOEVOEGEN
$timestamp
try {
    Add-DnsServerPrimaryZone -Name $domainname -ReplicationScope Forest -DynamicUpdate Secure -Erroraction Stop
    write-host "-= SUCCESFULLY ADDED IN $($domainname) AS DNS SERVER =-" -ForegroundColor green
    add-content $logfile "$($timestamp) - SUCCESFULLY ADDED IN $($domainname) AS DNS SERVER"
    }
catch{
    Write-Warning -Message $("FAILURE TO CREATE DNS SERVER FOR $($domainname). Error: "+ $_.exception.message)
    Break;
    }
